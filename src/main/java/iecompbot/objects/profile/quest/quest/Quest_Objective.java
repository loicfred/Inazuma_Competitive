...
public class Quest_Objective extends BaseObjective<Quest_Objective> {
    ...
    protected void IncrementProgress(String Type, int amount) {
        for (Objective O : list()) {
            if (O.getType().equals(Type)) {
                if (O.I == 1) {
                    Progress1 = O.Increment(amount);
                } else if (O.I == 2) {
                    Progress2 = O.Increment(amount);
                } else if (O.I == 3) {
                    Progress3 = O.Increment(amount);
                } else if (O.I == 4) {
                    Progress4 = O.Increment(amount);
                } else if (O.I == 5) {
                    Progress5 = O.Increment(amount);
                }
                Update();
            }
        }
    ...
    private Quest_Objective() {}
    public Quest_Objective(long questid, Objective... o) {
        this.QuestID = questid;
        init(List.of(o));
    }
    public Quest_Objective(long questid, List<Objective> o) {
        this.QuestID = questid;
        init(o);
    }
    ...
    public static Quest_Objective get(long id) {
        return getById(Quest_Objective.class, id).orElse(null);
    }
    public static Quest_Objective ofQuest(long questID) {
        return getWhere(Quest_Objective.class, "QuestID = ?", questID).orElse(null);
    }
