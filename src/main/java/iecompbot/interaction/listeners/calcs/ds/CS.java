...
public class CS {
    ...
    public static void CS_LoadCommands(SlashCommandInteractionEvent event) {
        if (!event.isAcknowledged()) {
            if (CanUseCommand(event)) {
                try {
                    if (event.getName().contains("-cs")) {
                        event.deferReply().queue(M -> {
                            switch (event.getName()) {
                                case "build-cs-player" -> BuildCS(event, M);
                                case "filter-cs-players" -> FilterCS(event, M);
                                case "library-cs" -> LibraryCS(event, M);
                            }
                        });
                    }
                } catch (Exception e) {
    ...
    public static void FilterCS(SlashCommandInteractionEvent event, InteractionHook M) {
        if (event.getName().contains("players")) {
            CSFilter CSB = new CSFilter(BuildType.get(event.getOption("as-position").getAsString()), Element.get(event.getOption("team-element").getAsString()));

            if (event.getOption("miximax-name") != null) {
                CSB.setMixi(CSUtils.getPlayersByName(event.getOption("miximax-name").getAsString()).getFirst());
            }
            ...
            if (event.getOption("with-move") != null) {
                CSB.withMove(event.getOption("with-move").getAsString());
            }
    ...
    public static void BuildCS(SlashCommandInteractionEvent event, InteractionHook M) {
        try {
            if (event.getName().contains("player")) {
                CSPlayer P = CSUtils.getPlayersByName(event.getOption("player-name").getAsString()).getFirst();
                CSBuilder CSB = new CSBuilder(P, BuildType.get(event.getOption("as-position").getAsString()), Element.get(event.getOption("team-element").getAsString()));
    ...
    private static void DisplayPlayer(InteractionHook M, CSPlayer P) {
        try {
            if (P != null) {
                ...
                String kickreal = P.getKick(true, true, true) + " (" + P.getFreedomSpentKick() + ") `[" + P.getEquipmentBoost(DSStats.KICK) + "]`";
                String drireal = P.getDribble(true, true, true) + " (" + P.getFreedomSpentDribble() + ") `[" + P.getEquipmentBoost(DSStats.DRIBBLE) + "]`";
                String defreal = P.getBlock(true, true, true) + " (" + P.getFreedomSpentBlock() + ") `[" + P.getEquipmentBoost(DSStats.BLOCK) + "]`";
                String catchreal = P.getCatch(true, true, true) + " (" + P.getFreedomSpentCatch() + ") `[" + P.getEquipmentBoost(DSStats.CATCH) + "]`";
                String techreal = P.getTechnique(true, true, true) + " (" + P.getFreedomSpentTechnique() + ") `[" + P.getEquipmentBoost(DSStats.TECHNIQUE) + "]`";
                String speedreal = P.getSpeed(true, true, true) + " (" + P.getFreedomSpentSpeed() + ") `[" + P.getEquipmentBoost(DSStats.SPEED) + "]`";
                String stareal = P.getStamina(true, true, true) + " (" + P.getFreedomSpentStamina() + ") `[" + P.getEquipmentBoost(DSStats.STAMINA) + "]`";
                String luckreal = P.getLuck(true, true, true) + " (" + P.getFreedomSpentLuck() + ") `[" + P.getEquipmentBoost(DSStats.LUCK) + "]`";
                ...
                for (SlotMove SM : P.getMoves()) {
                    if (SM instanceof CSSpecialMove S) {
                ...
                M.editOriginalEmbeds(E.build()).setFiles(FileUpload.fromData(P.getImage(), "avatar.png")
                        //, FileUpload.fromData(P.ExportToNFFMFile(new File(TempDirectory + "/" + takeOnlyLetters(P.getName()) + ".INZ5")),  P.getName() + (P.getAura() != null ? "_" + P.getAura().getName() : "") + ".INZ5")
    ...
    public static void LibraryCS(SlashCommandInteractionEvent event, InteractionHook M) {
        if (event.getOption("type").getAsString().equals("Player")) {
            ViewPlayer(M, CSUtils.getPlayersByName(event.getOption("name").getAsString()));
        } else if (event.getOption("type").getAsString().equals("Move")) {
            ViewMove(M, List.of((CSSpecialMove) CSUtils.getMoveByName(event.getOption("name").getAsString())));
        } else if (event.getOption("type").getAsString().equals("Spirit")) {
            ViewSpirit(M, List.of(CSUtils.getSpiritByName(event.getOption("name").getAsString())));
        }
    }
    public static void ViewPlayer(InteractionHook M, List<CSPlayer> Players) {
        try {
            if (!Players.isEmpty()) {
    ...
    public static void ViewMove(InteractionHook M, List<CSSpecialMove> Moves) {
        try {
            if (!Moves.isEmpty()) {
                List<MessageEmbed> Es = new ArrayList<>();
                List<FileUpload> FU = new ArrayList<>();
                for (int i = 0; i < 5; i++) {
                    CSSpecialMove S = Moves.get(i);
                    EmbedBuilder E = new EmbedBuilder();
    ...
    public static void ViewSpirit(InteractionHook M, List<CSSpirit> Spirits) {
        try {
            if (!Spirits.isEmpty()) {
                List<MessageEmbed> Es = new ArrayList<>();
                List<FileUpload> FU = new ArrayList<>();
